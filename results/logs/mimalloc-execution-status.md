...
## Host-only stopping point

1. RPM deploy and four-way runtime banner gate — NOT_RUN because the board is
   occupied and this authorization explicitly stops at the host RPM.
   Run after a release signal: `scripts/deploy.sh`.
2. Four-variant board session — NOT_RUN because deployment is not authorized.
   Rerun: `SDB_TARGET=192.168.108.25 scripts/run_board.sh`.
3. Data-filled `report-mimalloc.md` — NOT_RUN because there is no
   `results/results-mimalloc.txt`; it is generated by step 2.

Execution stopped successfully at the authorized host-only boundary. No board
state was changed and no performance conclusion is asserted.
